...
import { User, LogOut, X, LayoutDashboard } from "lucide-react";
...
const Navbar = () => {
  ...
  const handleSignIn = () => {
    const err = signIn(authEmail, authPassword);
    if (err) {
      setAuthError(err);
    } else {
      setAuthError("");
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-xl">
        <div className="container mx-auto flex h-14 items-center justify-between px-4 sm:h-16">
          <Link to="/" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="flex items-center gap-2 sm:gap-3">
            <img src={logo} alt="Gaudiya Warriors Logo" className="h-8 w-8 rounded-full border-2 border-primary glow-red sm:h-10 sm:w-10" />
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-background/80 backdrop-blur-sm p-4" onClick={() => setShowAuthModal(false)}>
          <div className="relative w-full max-w-sm rounded-xl border border-border bg-card p-6 sm:p-8 glow-red" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="mb-2 text-center font-display text-lg font-bold text-primary text-glow sm:text-xl">LOGIN</h2>
            <p className="mb-5 text-center text-sm text-muted-foreground font-body">Sign in to access your dashboard</p>
            {authError && (
            ...
            <div className="mb-4">
              ...
              <input type="password" value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSignIn()} className="w-full rounded-lg border border-border bg-secondary/50 px-4 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none" placeholder="••••" />
            </div>
            <Button onClick={handleSignIn} className="w-full gradient-red font-heading text-sm font-semibold tracking-wide sm:text-base">
              Sign In
            </Button>
          </div>
        </div>
